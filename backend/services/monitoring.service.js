...
function getServerModule() {
  if (!serverModule) {
    try {
      ...
      logger.error('[MonitoringService] Failed to load server module', { error: err.message });
    }
  }
  return serverModule;
}
...
async function getWorkerStatus() {
  try {
    ...
    return {
      naverCafe: {
        status: checkProcessAlive(naverCafeInfo?.process) ? 'running' : 'stopped',
        pid: naverCafeInfo?.process?.pid || null,
        lastCheck: new Date().toISOString()
      },
      naverCafeClan: {
        status: checkProcessAlive(naverCafeClanInfo?.process) ? 'running' : 'stopped',
        pid: naverCafeClanInfo?.process?.pid || null,
        lastCheck: new Date().toISOString()
      },
      naverCafeBackfill: {
        status: checkProcessAlive(naverCafeBackfillInfo?.process) ? 'running' : 'stopped',
      ...
      discourseInzoi: {
        status: checkProcessAlive(discourseInzoiInfo?.process) ? 'running' : 'stopped',
        pid: discourseInzoiInfo?.process?.pid || null,
        lastCheck: new Date().toISOString()
      }
    };
  } catch (error) {
    logger.error('[MonitoringService] Failed to get worker status', { error: error.message });
    return {
      naverCafe: {
        status: 'unknown',
        pid: null,
...
async function createKeyword(data) {
  ...
  return queryOne('SELECT * FROM MonitoringKeyword WHERE id = ?', [result.lastInsertRowid]);
}
...
async function deleteKeyword(id) {
  const keyword = queryOne('SELECT * FROM MonitoringKeyword WHERE id = ?', [id]);
  if (!keyword) {
...
async function setConfig(key, value, description = null) {
  const existing = queryOne('SELECT * FROM MonitoringConfig WHERE key = ?', [key]);
  const now = new Date().toISOString();
...
async function triggerManualScan() {
  try {
    ...
    logger.info('[MonitoringService] Manual scan triggered', { pid: process.pid });
    ...
    logger.error('[MonitoringService] Failed to trigger manual scan', { error: error.message });
    throw error;
  }
}
...
async function triggerSlackNoticeCollection() {
  try {
    ...
    if (!workerRunning) {
      logger.info('[MonitoringService] Slack notice worker not running, attempting to start...');
      ...
      if (serverModule && serverModule.startSlackNoticeWorker) {
        try {
          ...
          if (workerProcess) {
            ...
            workerRunning = true;
            logger.info('[MonitoringService] Slack notice worker started', { pid: workerPid });
            
            // 워커가 시작될 때까지 잠시 대기
            await new Promise(resolve => setTimeout(resolve, 2000));
          } else {
            logger.warn('[MonitoringService] Failed to start Slack notice worker (configuration may be missing)');
          }
        } catch (err) {
          logger.error('[MonitoringService] Error starting Slack notice worker', { error: err.message });
        }
      } else {
        logger.warn('[MonitoringService] Cannot start Slack notice worker: server module not available');
      }
    }
    ...
    if (workerRunning) {
      ...
      logger.info('[MonitoringService] Manual Slack notice collection triggered', { 
        pid: workerPid,
        workerStarted 
      });
      ...
      logger.info('[MonitoringService] Manual Slack notice collection trigger flag set (worker not running)', {
        note: '워커가 시작되면 플래그를 감지하고 수집을 실행합니다'
      });
      return {
        success: true,
        message: '트리거 플래그가 설정되었습니다. 워커가 시작되면 자동으로 수집을 실행합니다.',
        workerRunning: false,
        workerStarted: false,
        timestamp: new Date().toISOString()
      };
    ...
    logger.error('[MonitoringService] Failed to trigger Slack notice collection', { error: error.message });
    throw error;
  }
}
